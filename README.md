## 👋 Hi, I’m **Tian Qi | 田琪**

Postdoctoral Researcher @ [**China Agricultural University (CAU)**](https://www.cau.edu.cn/)  
Remote Sensing · Crop Phenotyping · AI for Agriculture

---

### 🔬 About Me

I am a **postdoctoral researcher at China Agricultural University**.  
My research focuses on **remote sensing–driven crop phenotyping** and  
**data–model integration for agricultural applications**.

I work on integrating **UAV, satellite data, and machine learning**  
to support **crop monitoring, stress detection, and breeding research**.

---
### 🎓 Education

B.Sc., M.Sc., and Ph.D. all completed at **Northwest A&F University**,  
with a background in **agriculture, GIS, and remote sensing**.

---

### 🧭 Research Interests

- Crop phenotyping and stress monitoring  
- UAV and satellite remote sensing  
- Machine learning / deep learning for agriculture  
- Agricultural modeling and GIS  
- Model–data integration (remote sensing + crop models)

---

### 🛠 Tools & Skills

- **Programming**: Python, R  
- **Machine Learning**: PyTorch, Scikit-learn  
- **Remote Sensing**: UAV hyperspectral, Sentinel, Landsat  
- **Data & GIS**: Jupyter, QGIS, GDAL  
- **System**: Linux (Ubuntu)

---

### 🤝 Collaboration

I am interested in collaboration on:

- Crop simulation models (e.g., DSSAT, APSIM)  
- UAV–satellite data fusion  
- AI-based crop monitoring and phenotyping  
- Field experiments combined with remote sensing data

International collaboration is welcome.

---

### 📫 Contact

- GitHub: https://github.com/tiqi0326 
- Email: tq0326@gmail.com  
- ORCID: [https://orcid.org/0000-0001-8897-1714](https://orcid.org/0000-0001-8897-1714)
